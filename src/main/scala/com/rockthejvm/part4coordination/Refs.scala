...
object Refs extends ZIOAppDefault {
  ...
  def demoConcurrentWorkImpure(): UIO[Unit] = {
    ...
    val effects: List[UIO[Unit]] = List("I love ZIO", "This Ref is cool", "Daniel writes a LOT of code!").map(task)
  ...
  def demoConcurrentWorkPure(): UIO[Unit] = {
    ...
    for {
      ...
      _ <- ZIO.collectAllParDiscard(
        List("I love ZIO", "This Ref is cool", "Daniel writes a LOT of code!")
