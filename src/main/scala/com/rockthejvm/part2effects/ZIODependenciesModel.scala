...
object ZIODependenciesModel {
  ...
  class EmailService {
    def email(user: User): Task[Unit] = ZIO
      .succeed(
        println(
  ...
  object EmailService {
    ...
    val live: ZLayer[Any, Nothing, EmailService] = ZLayer.fromFunction(create _)
  }
  ...
  object ConnectionPool {
    def create(nConnections: Int): ConnectionPool = new ConnectionPool(
      nConnections
    )

    def live(nConnections: Int): ZLayer[Any, Nothing, ConnectionPool] =
      ZLayer.succeed(create(nConnections)) // or
      // ZLayer.succeed(nConnections) >>> ZLayer.fromFunction(create _)
  }
